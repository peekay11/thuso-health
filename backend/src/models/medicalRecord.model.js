...
class MedicalRecordModel {
  ...
  static create(recordData) {
    ...
    const newRecord = {
      ...
      file_url_r2: recordData.file_url_r2 || null,
  ...
  static updateConsent(patientId, consentData) {
    const user = db.users.find(u => u.id === patientId);
    if (!user) return null;
    if (consentData.consentPin !== undefined) {
      user.consentPin = consentData.consentPin;
    }
    if (consentData.isAccessGranted !== undefined) {
      user.isAccessGranted = consentData.isAccessGranted === true;
    }
    if (consentData.language !== undefined) {
      user.language = consentData.language;
    }
    saveDb();
    return {
